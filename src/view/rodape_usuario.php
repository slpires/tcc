...
/* [ROTA ATUAL] Utilizada para lógica de exibição do botão Voltar */
$rota_atual = isset($_GET['r']) ? (string) $_GET['r'] : '';
...
/*
 * [AJUSTE] Montagem segura das URLs de navegação
 *  - Preferência: $action_base (quando definido em paths.php)
 *  - Fallback: $base_url/index.php?r=...
 *  - Fallback mínimo: ./index.php?r=...
 */
...
/* URL do painel de módulos */
if (isset($action_base) && $action_base !== '') {
    $painel_href = $action_base . '?r=painel';
} elseif (isset($base_url) && $base_url !== '') {
    $painel_href = rtrim($base_url, '/') . '/index.php?r=painel';
} else {
    $painel_href = './index.php?r=painel';
}
$painel_href = htmlspecialchars($painel_href, ENT_QUOTES, 'UTF-8');
?>
<footer class="footer-version app-footer">
  <small>
    ...
    <!-- Navegação institucional: Voltar ao Painel e Sair do Sistema -->
    <div style="text-align:center; margin-top:0.8rem;">
      <?php if ($rota_atual !== 'painel'): ?>
        <a href="<?= $painel_href ?>"
           class="btn-voltar-painel"
           aria-label="Voltar ao painel de módulos do sistema"
      ...
      <a href="<?= $logout_href ?>"
         ...
         style="display:inline-block; padding:0.5em 1.2em; border-radius:6px;
                background-color:#45763f; color:#fff; text-decoration:none;
                font-weight:600;">
         Sair do Sistema
      </a>
    </div>
